/**
 * last year of commit data including daily and weekly - output as bar chart using d3 with interaction and sort
 **/
...
function sortBars(container) {
d3.select(container + " svg").selectAll("rect")
	.sort(function(a, b) {
...
function buildBars(gitData, barLabel, container) {
...
//get the bar's x/y values
var xPosition = parseFloat(d3.select(this).attr("x")) + xScale.rangeBand() / 2;
var yPosition = parseFloat(d3.select(this).attr("y")) / 2 + h / 5;
...
//enable sort on click
.on("click", function() {
		sortBars(container);
		svg.selectAll("text").remove();
		var sortedData = weekly_totals.sort();
		//append the data labels
		buildText(sortedData, container);
	})
	.attr("x", w)							
	.attr("y", function(d) {				
	})
...
buildBars(daily_totals, "Day", "#test-container");
	
//build the bar chart - daily totals		
outputText("Total commits per week - interactive sort", "#test-container2");
buildBars(weekly_totals, "Week", "#test-container2");
//append the data labels
